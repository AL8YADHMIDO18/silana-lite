...
const startServer = async () => {
  ...
  if (!isPortAvailable) {
    console.error(chalk.red(`Port ${port} is already in use. Please choose another port.`));
    process.exit(1);
  }
  ...
  fs.readdir(pluginsFolder, async (err, files) => {
    ...
    try {
      console.log(
        chalk.bgGreen(
          chalk.white(
            `Library Baileys Version ${(await fetchLatestBaileysVersion()).version} Has been installed`
          )
        )
      );
    } catch (e) {
